<div v-if="noteInfo" class="calendar-note-wrap">
    ...
    <div class="calendar-note-bottom">
        <span class="time"><i class="time-icon"></i><span>{{ noteInfo.formatStartTime }} - {{ noteInfo.formatEndTime }}</span></span>
        <span class="remind-time"><i class="remind-time-icon"></i><span>{{ noteInfo.formatRemindTime }}</span></span>
